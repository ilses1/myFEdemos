Based on the codebase analysis and web search results, the main reason for the "slow loading" of the Animation Collection page is likely the **large bundle size** caused by the `react-syntax-highlighter` library, which is currently importing _all_ language definitions by default.
