...
**1. Modify `config/routes.ts`** Add a custom `devOnly: true` property to the specific routes that should be hidden in production.

- Add `devOnly: true` to the '前端草稿' (Frontend Draft) route definition.
- Add `devOnly: true` to the '图表' (Charts) route definition.

**2. Modify `config/config.ts`** Implement logic to filter out these routes during the production build.
